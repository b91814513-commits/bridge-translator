...
- 查词与收藏分离：悬停只查词，点击字幕单词或查词浮层书签后才加入生词本。
- 生词持久化保存，并沿用现有同步机制；每条记录保留释义、字幕原句、译文、视频和时间点。
- YouTube 侧栏按当前视频筛选生词，支持时间跳转和移除；设置页支持搜索、全部词汇 / 按视频管理及四种格式导出。
- 同词同视频自动合并，同词跨视频分别归档；旧数据和 TXT / CSV 导入保持兼容。
- 撤销提示改为贴近查词浮层或字幕单词显示，并在视口边缘自动调整位置。
