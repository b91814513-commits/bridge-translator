import {
  ...
  defaultWebSummaryPrompt,
  defaultVideoSummaryPrompt,
  API_SPE_TYPES,
} from "./api";
...
export const PROMPT_SLUG_WEB_SUMMARY = "web-page-summary";
export const PROMPT_SLUG_VIDEO_SUMMARY = "video-summary";
...
export const PROMPT_CATEGORY_SUMMARY = "summary prompt";
...
export const DEFAULT_WEB_SUMMARY_PROMPT_SLUG = PROMPT_SLUG_WEB_SUMMARY;
export const DEFAULT_VIDEO_SUMMARY_PROMPT_SLUG = PROMPT_SLUG_VIDEO_SUMMARY;
...
export const PRESET_PROMPTS = [
  ...
  {
    slug: PROMPT_SLUG_WEB_SUMMARY,
    category: PROMPT_CATEGORY_SUMMARY,
    nameKey: "prompt_web_summary",
    name: "Web Page Summary",
    systemPrompt: defaultWebSummaryPrompt,
    userPrompt: "",
  },
  {
    slug: PROMPT_SLUG_VIDEO_SUMMARY,
    category: PROMPT_CATEGORY_SUMMARY,
    nameKey: "prompt_video_summary",
    name: "Video Summary",
    systemPrompt: defaultVideoSummaryPrompt,
    userPrompt: "",
  },
];
...
export function getPromptCategoryDisplayName(category, i18n) {
  ...
  const keyMap = {
    ...
    [PROMPT_CATEGORY_SUMMARY]: "prompt_category_summary",
  };
 ...
 * @returns {Array}
 */
export function getSummaryPromptOptions(prompts = []) {
  return getPromptOptions(prompts, PROMPT_CATEGORY_SUMMARY);
}
...
export function removePromptReferences(setting = {}, promptSlug) {
  ...
  const transApis = (
    ...
    if (hasPromptReference(api, "summaryPromptSlug", promptSlug)) {
      nextApi = {
        ...nextApi,
        summaryPromptSlug: DEFAULT_WEB_SUMMARY_PROMPT_SLUG,
      };
      hasApiChanges = true;
    }

    if (hasPromptReference(api, "videoSummaryPromptSlug", promptSlug)) {
      nextApi = {
        ...nextApi,
        videoSummaryPromptSlug: DEFAULT_VIDEO_SUMMARY_PROMPT_SLUG,
      };
      hasApiChanges = true;
    }
...
export function resolveApiPromptSettings(
  ...
  const summaryPromptSlug = getPromptFieldValue(
    nextApiSetting,
    "summaryPromptSlug",
    DEFAULT_WEB_SUMMARY_PROMPT_SLUG
  );
  const summaryPrompt = findPromptBySlugOrDefault(
    userPrompts,
    summaryPromptSlug,
    DEFAULT_WEB_SUMMARY_PROMPT_SLUG
  );

  // 总结提示词始终展开为专用的网页总结提示词（默认或用户配置），
  // 与 batch/nobatch/dict/subtitle 保持一致；否则调用方会误回退到翻译用的
  // systemPrompt（JSON 批量翻译提示词），导致总结返回 JSON 而非可读的 Markdown。
  if (summaryPrompt) {
    nextApiSetting.summaryPromptSlug = summaryPrompt.slug;
    nextApiSetting.summaryPrompt = summaryPrompt.systemPrompt;
  }

  const videoSummaryPromptSlug = getPromptFieldValue(
    nextApiSetting,
    "videoSummaryPromptSlug",
    DEFAULT_VIDEO_SUMMARY_PROMPT_SLUG
  );
  const videoSummaryPrompt = findPromptBySlugOrDefault(
    userPrompts,
    videoSummaryPromptSlug,
    DEFAULT_VIDEO_SUMMARY_PROMPT_SLUG
  );

  // 视频总结提示词同理，始终展开为专用的视频总结提示词。
  if (videoSummaryPrompt) {
    nextApiSetting.videoSummaryPromptSlug = videoSummaryPrompt.slug;
    nextApiSetting.videoSummaryPrompt = videoSummaryPrompt.systemPrompt;
  }
