...
async function* fetchStreamGM(
  ...
  const gmRequest = window.BRIDGE_GM?.xmlHttpRequest || GM.xmlHttpRequest;
...
async function* fetchStreamViaPort(input, init, opts) {
  ...
  // 标记 background 是否已通过 done/error 正常结束流；
  // 正常结束后由我方 finally 主动断开 Port，此时的 onDisconnect 应被忽略。
  let settled = false;
  ...
  port.onDisconnect.addListener(() => {
    ...
    // 已收到 done/error 的正常收尾无需处理；否则说明 background/Service Worker
    // 在流结束前意外断开（如 MV3 空闲回收），必须让消费方以错误结束迭代，
    // 从而触发上层的降级重试，避免 for-await 永久挂起导致界面一直加载。
    if (settled) return;
    asyncQueue.error(
      new Error(
        lastError?.message ||
          "Background stream port disconnected before completion"
      )
    );
  });
