    ...
    (async () => {
      if (isGm) {
        ...
        for (;;) {
          ...
          if (++i > 8) {
            setError(
              "Time out. Please confirm whether to install or enable Bridge Translator GreaseMonkey script? 连接超时，请检查是否安装或启用桥接翻译油猴脚本。"
            );
            return;
          }
    ...
    return (
      <center>
        <Divider>
          ...
          >{`Bridge Translator v${process.env.REACT_APP_VERSION}`}</Link>
        </Divider>
