...
import {
  ...
  MSG_UPDATE_ICON,
  MSG_SHA256,
  BRIDGE_SUMMARIZE_PAGE,
  BRIDGE_SUMMARIZE_TRIGGER,
  BRIDGE_VIDEO_SUMMARY,
} from "./config";
import {
...
/**
 * 根据 API 类型构建总结请求体。
 * 支持 OpenAI 兼容格式、Claude、Gemini 等主流 AI 接口。
 * @param {Object} apiSetting 解析后的 API 配置
 * @param {string} systemPrompt 系统提示词
 * @param {string} userContent 用户提供的网页文本
 * @returns {Object} { url, init } 请求参数
 */
function buildSummaryRequest(apiSetting, systemPrompt, userContent) {
  const { apiType, url, key, model, maxTokens = 4096, temperature = 0.3 } = apiSetting;
  ...
  // 默认：OpenAI 兼容格式（适用于 DeepSeek、OpenAI、Ollama、OpenRouter、SiliconFlow 等）
  return {
    url,
    init: {
      method: "POST",
...
/**
 * 处理网页总结请求。
 * 读取当前设置，构建 API 请求，调用 AI 生成总结并返回结果。
 * @param {Object} args { content: string } 网页文本内容
 * @returns {Promise<Object>} { summary: string } 或 { error: string }
 */
async function handleWebSummary({ content } = {}) {
  try {
    if (!content?.trim()) {
    ...
    const aiApiTypes = new Set([
      "OpenAI", "Gemini", "Gemini2", "Claude", "DeepSeek", "Ollama",
      "OpenRouter", "SiliconFlow", "AliyunBailian", "Cerebras",
      "Zai", "ePhoneAI", "XiaomiMimo", "OpenCodeGo", "CloudflareAI",
      "Custom", "AzureAI",
    ]);
    ...
    if (!summary) {
      kissLog("Summary API returned empty response", response);
      return { error: "Empty response from summary API" };
    }

    return { summary };
  } catch (err) {
    kissLog("handleWebSummary error", err);
...
/**
 * 处理视频总结请求。
 * 读取当前设置，构建 API 请求，调用 AI 生成视频总结并返回结果。
 * @param {Object} args { subtitles: string } 带时间戳的字幕文本
 * @returns {Promise<Object>} { summary: string } 或 { error: string }
 */
async function handleVideoSummary({ subtitles } = {}) {
  try {
    if (!subtitles?.trim()) {
      return { error: "No subtitle data to summarize" };
    }
    ...
    if (!activeApi) {
      return { error: "No active AI API configured for video summary" };
    }
    ...
    kissLog("handleVideoSummary error", err);
    return { error: err?.message || "Failed to generate video summary" };
  }
}
...
browser.commands?.onCommand?.addListener?.((command) => {
  switch (command) {
    ...
    case CMD_OPEN_SEPARATE_WINDOW:
      ...
      break;
    case "summarize_page":
      sendTabMsg(BRIDGE_SUMMARIZE_TRIGGER);
      break;
    default:
  }
});
