...
import { THEME_DARK, THEME_LIGHT, THEME_AUTO } from "../config";
...
export default function Theme({ children, options = {}, styles = {} }) {
  // 获取当前用户设置的深色模式：'light', 'dark' 或 'auto'
  ...
  const theme = useMemo(() => {
    ...
    const isDarkMode =
      darkMode === THEME_DARK ||
      (darkMode === THEME_AUTO && systemMode === THEME_DARK);
    ...
    const lightPalette = {
      ...
      background: {
        default: "#FFF0F5", // 淡粉色背景（薰衣草腮红）
        paper: "#FFFFFF", // 白色纸面
      },
      text: {
        primary: "#37474F", // 深灰蓝文字，确保对比度
        secondary: "#607D8B",
      },
    };

    // 构建 MUI 主题对象，深色模式沿用 MUI 默认暗色调色板，浅色模式使用浅粉色调色板
    const baseTheme = {
      palette: isDarkMode
        ? {
            mode: THEME_DARK,
    ...
    // 合并用户自定义的 options（如 shape、spacing 等），但确保 palette 不被覆盖
    return createTheme({
      ...options,
      ...baseTheme,
    });
